  ...
  const renderItem = ({ item }: { item: Order }) => {
    return (
      <View className="gap-2 p-2 border-2 rounded-3xl justify-center shadow-md shadow-black bg-white">
        ...
        {item.status.toLocaleLowerCase() === "preparando" && (
          <CustomButton
            title="Cambiar a en camino"
            onPress={() => updateOrderStatus(item.id, "En camino")}
          />
        )}
        {item.status.toLocaleLowerCase() === "en camino" && (
          <CustomButton
            title="Cambiar a entregado"
            onPress={() => updateOrderStatus(item.id, "Entregado")}
          />
        )}
      </View>
    );
